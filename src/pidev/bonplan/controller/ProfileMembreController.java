...
btnChangerEmail.setVisible(false);
 ...
 Alert alert = new Alert(Alert.AlertType.ERROR);
        ...
        } else {serviceCryptage cryptage = new serviceCryptage();
            String passCrypt = cryptage.cryptWithMD5(newPassword.getText());
       User user = new User();
...
//•	Contains at least 1 special character from the set: !@#$%^&*
    ...
    private void VerifPasswordActuelle(KeyEvent event) {
        ...
        if (event.getCode() == KeyCode.ENTER) {
            ...
            String passCrypt = cryptage.cryptWithMD5(passwordActuelle.getText());
            System.out.println(passCrypt);
            System.out.println(Integer.parseInt(IdUser.getText().trim()));
            if (myServices.verifMotDePasseExistantUser(passCrypt, Integer.parseInt(IdUser.getText().trim()))) {
                passwordActuelle.setVisible(false);
